...
# not used anymore. Used to allow closing of some dialogs with esc key. Got in the way of using enter as mouse click
    ...
    def update_units
      if (Plugin.model_manager.units_system == "SI")
        Plugin.model_manager.units_hash.keys.each { |key| set_element_value(key, Plugin.model_manager.units_hash[key][0]) }
      elsif (Plugin.model_manager.units_system == "IP")
        Plugin.model_manager.units_hash.keys.each { |key| set_element_value(key, Plugin.model_manager.units_hash[key][1]) }
      end
    end
